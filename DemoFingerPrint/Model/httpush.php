...
while ($fecha_bd <= $fecha_actual) {
    $sql = "SELECT update_time FROM huellas_temp ORDER BY update_time DESC LIMIT 1";
...
//     $reponse["id"] = base64_decode($rows[$i]['pc_serial']);
//     $reponse["timestamp"] = strtotime($rows[$i]['update_time']);
//     $reponse["texto"] = $rows[$i]['texto'];
//     $reponse["statusPlantilla"] = $rows[$i]['statusPlantilla'];
//     $reponse["nombre"] = $rows[$i]['nombre'];
//     $reponse["documento"] = $rows[$i]['documento'];
//     $reponse["imgHuella"] = $rows[$i]['imgHuella'];
//     $reponse["tipo"] = $rows[$i]['opc'];
// }
$reponse = array();
$reponse["id"] = base64_decode($rows[0]['pc_serial']);
